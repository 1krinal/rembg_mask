...
async def download_image_and_process(image_url: str):
    ...
    with open(f"{OVERLAY}overlay_{filename}", "wb") as output_file:
    ...
    mask = f"mask_o_{filename}"
    cv2.imwrite(f"{MASK}{mask}",overlay_mask)
...
async def process_image(filename: str,file: UploadFile = File(...)  ):
    ...
    try:
        ...
        overlay_filename = f'{OVERLAY}B&W_{filename}'
        ...
        return {'file_path': str(overlay_filename),'merge_image':f"{OVERLAY}merge_{filename}"}
